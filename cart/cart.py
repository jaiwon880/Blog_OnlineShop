from decimal import Decimal #숫자 형태로 출력
from django.conf import settings # settings 파일안에 있는 것 가져오기
from shop.models import Product
from coupon.models import Coupon

class Cart(object):
    # 초기화 , Django view에서 사용한 리퀘스트로 그 안에 session 정보가 들어있음
    def __init__(self, request):
        self.session = request.session
        cart = self.session.get(settings.CART_ID) #settings에 CART_ID를 만들어야 함.
        if not cart:
            #cart 정보가 없을 때는 새로운 dictionary 만들어줌
    ...
    def __iter__(self): # for 문 등 사용할 때 어떤 요소들을 건내줄 것인지 지정
        ...
        for item in self.cart.values():
            # 제품 가격, Decimal: 숫자형으로 바꿔서 item에 넣어둠
        ...
        #제품 장바구니에 넣기
        # is_update 제품정보를 업데이트 하는지 아닌지 확인하는 것
