$(function (){
    ...
    IMP.init('imp92798925'); // iamport 관리자 페이지에 있는 가맹점 식별 코드를 입력해야 함

    $('.order-form').on('submit',function (e) { //order-form에 submit 버튼을 눌렀을 때 function을 동작시킴
        var amount =parseFloat($('.order-form input[name="amount"]').val().replace(',','')); //결제금액
        ...
        if (merchant_id !== ''){ // !==: type까지 같은지 체크
            ...
            IMP.request_pay({ //결제 요청 부분, iamport 매뉴얼에서 정의해준 포맷
...
function AjaxCreateOrder(e){
    e.preventDefault(); //form이 submit 되는 것을 막아줌
    var order_id = '';
    ...
    var request = $.ajax({
        ...
        async: false,
        data: $('.order-form').serialize() //serialize() 통해 form의 객체들을 한 번에 가져옴
    });
